...
for g,gamma in enumerate(args.Ogammas):
    ...
    fn = f'g2_saved_models_MIPT/MIPT_QuAN4_Tbt2_L20_zerosz_one_g0.100vs0.850_f832_run_106/'+\
    ...
    for t in args.ts:
        ...
        fn = f'MIPT/counts_M={fraction},L={L},add={add},init={init},gamma={gamma},perm=t={t}.npz'
        prob = np.load(fn)['c']
        ...
        np.savez(f'newnewattnavglarge_t={t}_x_{run}_{gamma}', avg=attn_avg[run,g], std=attn_std[run,g])
        del ptraj_outer


    #for run in args.runs:
    #    np.savez(f'newnewattnavg_t={t}_x_{run}', avg=attn_avg[run], std=attn_std[run])

...
def get_arguments():
    ...
    parser.add_argument("-gamma1", type = float, default = 0.10)
    parser.add_argument("-gamma2", type = float, default = 0.85)
...
def main():
    ...
    if n_bitstring <= 50000: 
        basic_size = 40000 // 4 // args.setsize * args.setsize 
    else:
    ...
    x1, x2, x3v, x4v = gen_data_wkmsmt_embedding(args, gammas, args.L, n_bitstring=n_bitstring, basic_size=basic_size, maxnfile = 50, add=f'_zerosz_p_{args.protocol}{args.noise}')
    ...
    if args.test==True:
        test_loaders = []
        if args.setsize!=None:
            x3v = x3v.reshape(-1, Nt); n_truncate = x3v.shape[0] - x3v.shape[0]%args.setsize
            x3v = x3v[:n_truncate].reshape(-1, args.setsize, Nt)
            x4v = x4v.reshape(-1, Nt); n_truncate = x4v.shape[0] - x4v.shape[0]%args.setsize
            x4v = x4v[:n_truncate].reshape(-1, args.setsize, Nt)
        test_loaders.append(DataLoader(dataset = testing_set2(x3v), batch_size=args.batchsize * 10, shuffle=True))
        for gamma in gammas_t:
            x5 = gen_data_wkmsmt_1gamma_embedding(args, args.L, gamma, n_bitstring=np.min([basic_size, 50000]), maxnfile=150).astype(datatype)
            test_loaders.append(DataLoader(dataset = testing_set2(x5), batch_size=args.batchsize * 10, shuffle=True))
            x5s.append(x5)
        test_loaders.append(DataLoader(dataset = testing_set2(x4v), batch_size=args.batchsize * 10, shuffle=True))
    ...
    while restart:
        ...
        for epoch in range (total_epoch):
            ...
            # validation
            if total_epoch > 400: epochcriteria = epoch % (total_epoch//400)
            ...
            if epochcriteria ==0:
                ...
                for _,valdata in enumerate(val_loader):
                    ...
                    if 'test' in args.modelnum: val_outputs, val_output_2 = model.module(val_inputs)
                    else: val_outputs = model.module(val_inputs)
                    del val_inputs
                    ...
                    if 'test' in args.modelnum: val_output_2_0 += val_output_2[m0]; val_output_2_1 += val_output_2[m1]
                ...
                if 'test' in args.modelnum:
                    val_output_2_0 /= len(val_loader)
                    val_output_2_1 /= len(val_loader)
                ...
                if 'test' in args.modelnum: np.savetxt(f_val, np.array([val_acc0, val_acc1, val_loss0, val_loss1, val_output_2_0, val_output_2_1]), newline=" ")
                else: np.savetxt(f_val, np.concatenate([val_acc0, val_acc1]), newline=" ")
                f.write("\n"); f.close()
                f_val.write("\n"); f_val.close()

                print(f'L={args.L}, set={args.setsize}, {args.gamma1}vs{args.gamma2}, frac={args.fraction} - [#{epoch}/{total_epoch}], lr = {learningratearg}')

...
def gen_data_wkmsmt_State(args, gamma, L, n_bitstring, verbose=True, basic_size=5000, maxnfile=100):
    nt = 2*L*L
    dir_path = args.prefix
    path = dir_path+f'/Data_src/StateDistinguishing/L_{L}/wk_msmt_traj_{L}_'

    # class-0 for training
    x1_ = []
    x3t_ = []
    data = np.load(path+f'g{gamma:.3f}_s0_0.npz')
    a = data['s'].astype(bool)
    x1 = a.reshape(a.shape[0], nt)
    for circi in range(1,maxnfile):
        fpath = path+f'g{gamma:.3f}_s0_{circi}.npz'
        if not os.path.isfile(fpath): break
        data = np.load(fpath)
        a = data['s'].astype(bool)
    ...
    x2_ = []
    x4t_ = []
    data = np.load(path+f'g{gamma:.3f}_s1_0.npz')
    a = data['s'].astype(bool)
    x2 = a.reshape(a.shape[0], nt)
    for circi in range(1,maxnfile):
        fpath = path+f'g{gamma:.3f}_s1_{circi}.npz'
        if not os.path.isfile(fpath): break
        data = np.load(fpath)
        a = data['s'].astype(bool)
        xx = a.reshape(a.shape[0], nt)
    ...
    if verbose:
        print(f'gen_data_wkmsmt_State from {path}: {gamma:.2f}')
...
def gen_data_wkmsmt_State_embedding(args, gamma, L, n_bitstring, basic_size=5000, maxnfile=100):
    _x1, _x2, _x3t, _x4t = gen_data_wkmsmt_State(args, gamma, L, n_bitstring, verbose=True, basic_size=basic_size, maxnfile=maxnfile, add=add)
    ...
    print(f'gen_data_wkmsmt_State_embedding: {gamma:.2f}')
